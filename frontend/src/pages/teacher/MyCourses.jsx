...
function MyCourses() {
  ...
  const startVideoUpload = async () => {
    ...
    if (!selectedVideo) {
      return toast.error("Select a video from gallery");
    }

    const isLive = Boolean(selectedVideo.isLiveSession);
    if (isLive) {
      const date = String(videoUpload.liveStartDate || "").trim();
      const time = String(videoUpload.liveStartTime || "").trim();
      if (!date) return toast.error("Select live session start date");
      if (!time) return toast.error("Select live session start time");
    }
    ...
    try {
      ...
      await saveLectureContentMutation.mutateAsync({
        ...
        data: {
          ...
          // In gallery mode, use live/recorded settings from gallery video.
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        {coursesQuery.isLoading ? Array.from({ length: 6 }).map((ignore, idx) => <SkeletonCard key={`s-${idx}`} />) : null}
